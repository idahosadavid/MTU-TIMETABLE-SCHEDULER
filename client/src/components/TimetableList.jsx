...
const TimetableList = () => {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-gradient-to-r from-[#4c1d95] via-[#6d28d9] to-[#4c1d95] rounded-2xl p-6 sm:p-8 text-white shadow-lg overflow-hidden relative">
                ...
                <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-purple-200 text-sm mt-1">Create and manage conflict-free schedules for Mountain Top University</p>
                    </div>
                    <button
            ...
            {/* Setup guidance — shown when essential data is missing */}
            {(stats.totalDepartments === 0 || stats.totalVenues === 0 || stats.totalLecturers === 0) && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
                    <svg className="w-5 h-5 text-amber-500 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
                    </svg>
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-amber-800">Complete your setup before creating timetables</p>
                        <p className="text-xs text-amber-700 mt-0.5">
                            {(() => {
                                const missing = [
                                    stats.totalDepartments === 0 && 'Departments',
                                    stats.totalVenues === 0 && 'Venues',
                                    stats.totalLecturers === 0 && 'Lecturers',
                                ].filter(Boolean);
                                return `${missing.join(', ')} ${missing.length === 1 ? 'has' : 'have'} not been configured yet.`;
                            })()}
                        </p>
                    </div>
                    <button onClick={() => navigate('/admin')} className="shrink-0 px-3 py-1.5 bg-amber-600 text-white rounded-lg text-xs font-medium hover:bg-amber-700 transition-colors">
                        Go to Setup
                    </button>
                </div>
            )}
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden lg:table-cell">Semester</th>
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden xl:table-cell">Modified</th>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4 whitespace-nowrap">
                                            {timetable.status === 'generated' ? (
                                                <span className="px-2.5 py-1 inline-flex items-center gap-1 text-xs leading-4 font-medium rounded-md bg-emerald-100 text-emerald-700">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
                                                    Generated
                                                </span>
                                            ) : (
                                                <span className="px-2.5 py-1 inline-flex items-center gap-1 text-xs leading-4 font-medium rounded-md bg-slate-100 text-slate-500">
                                        ...
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-slate-500 hidden xl:table-cell">
