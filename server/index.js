...
const normalizeUploadKey = (obj, key) => {
    return obj[key] || obj[Object.keys(obj).find(k => k.toLowerCase() === key.toLowerCase())];
};

const mapUploadedCourseRow = (row, customFields) => {
    const code = normalizeUploadKey(row, 'Course Code') || row.code;
    const title = normalizeUploadKey(row, 'Course Title') || row.title;
    const department = normalizeUploadKey(row, 'Department') || row.department;
    const level = parseInt(normalizeUploadKey(row, 'Level') || row.level);
    const lecturersStr = normalizeUploadKey(row, 'Lecturers') || row.lecturers || '';
    const lecturers = lecturersStr.split(',').map(l => l.trim());
    const units = parseInt(normalizeUploadKey(row, 'Units') || row.units);
    const semester = normalizeUploadKey(row, 'Semester') || row.semester || 'First';
    const type = normalizeUploadKey(row, 'Type') || row.type || 'Lecture';
    const compulsoryRaw = normalizeUploadKey(row, 'Compulsory') ?? normalizeUploadKey(row, 'Is Compulsory') ?? row.is_compulsory ?? row.compulsory;
    const is_compulsory = String(compulsoryRaw || '').toLowerCase() === 'true'
        || String(compulsoryRaw || '').toLowerCase() === 'yes'
        || String(compulsoryRaw || '') === '1';
    const preferred_day = normalizeUploadKey(row, 'Preferred Day') || row.preferred_day || 'AUTO';
    const preferred_time = normalizeUploadKey(row, 'Preferred Time') || row.preferred_time || 'AUTO';
    const venue = normalizeUploadKey(row, 'Venue') || row.venue || '';
    const durationHours = parseFloat(normalizeUploadKey(row, 'Duration') || row.duration || 1);
    const duration = durationHours * 60;
    const student_count = parseInt(normalizeUploadKey(row, 'Student Count') || row.student_count || 0);

    const custom_data = {};
    customFields.forEach(field => {
        const value = normalizeUploadKey(row, field.label) || normalizeUploadKey(row, field.name);
        if (value !== undefined) {
...
const parseTextToObjects = (text) => {
    ...
    if (lines[0].toLowerCase().includes('code') || lines[0].toLowerCase().includes('title')) {
        ...
        headers = ['code', 'title', 'department', 'level', 'lecturers', 'units', 'semester', 'type', 'preferred_day', 'preferred_time', 'venue', 'duration', 'student_count'];
    }
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
    ...
    const processData = async (data) => {
        try {
            ...
            let successCount = 0;
            let errorCount = 0;
            const timetable_id = req.body.timetable_id || req.query.timetable_id;
            ...
            for (const row of data) {
                try {
                    const mappedRow = mapUploadedCourseRow(row, customFields);
                    const {
                        code,
                        title,
                        department,
            ...
            fs.unlinkSync(filePath);
            res.json({ message: `Processed ${successCount} courses. Errors: ${errorCount}` });
        } catch (err) {
    ...
    try {
        if (fileExt === 'csv') {
            ...
            const parsedData = parseTextToObjects(data.text);
            void processData(parsedData);
        } else if (fileExt === 'doc' || fileExt === 'docx') {
            const result = await mammoth.extractRawText({ path: filePath });
            const parsedData = parseTextToObjects(result.value);
            void processData(parsedData);
        } else {
            fs.unlinkSync(filePath);
            res.status(400).json({ error: 'Unsupported file format. Use CSV, Excel, PDF, or Word.' });
        }
    } catch (err) {
